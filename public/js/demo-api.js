...
(() => {
  ...
  const settings = {
    ...
    weather_label: 'Demo City', interest_monthly: 10, interest_day: 1, coin_name: 'Mom Coins', coins_per_chore: 2, coins_per_dollar: 35, game_coins_per_minute: 0.5,
  ...
  const rewards = [
    ...
    { id: 4, title: 'One Dollar Bill', coins: 35, emoji: '💵', notes: 'Real money: a parent hands you $1', active: 1, sort_order: 1 },
  ...
  const gamesWindow = () => (settings.games_unlocked_day === ymd(today) ? { open: true, unlocked: true } : { open: true });
  const rateNow = () => (settings.games_free_day === ymd(today) ? 0 : settings.game_coins_per_minute);
  ...
  function route(method, path, query, body) {
    ...
    if (seg[0] === 'finance' && seg[2] === 'cash-in' && method === 'POST') { const id = num(seg[1]); const rate = num(settings.coins_per_dollar) || 35; const d = num(body.dollars); const c = d * rate; if (d < 1 || c > coinBalance(id)) return { status: 400, body: { error: `That needs ${c} coins` } }; coins.push({ id: nextId++, member_id: id, amount: -c, note: `💵 Cashed in for $${d}.00`, created_at: nowIso() }); tx.push({ id: nextId++, member_id: id, type: 'deposit', account: 'cash', amount_cents: d * 100, note: `Cashed in ${c} coins`, created_at: nowIso() }); return { cash_cents: balance(id, 'cash'), coins: coinBalance(id) }; }
    if (seg[0] === 'finance' && seg[2] === 'invest' && method === 'POST') { const id = num(seg[1]); const c = Math.min(Math.abs(num(body.amount_cents)), balance(id, 'cash')); if (c > 0) { tx.push({ id: nextId++, member_id: id, type: 'transfer', account: 'cash', amount_cents: -c, note: 'Invested with Dad (30-day promise)', created_at: nowIso() }); tx.push({ id: nextId++, member_id: id, type: 'transfer', account: 'invested', amount_cents: c, note: 'Invested with Dad (30-day promise)', created_at: nowIso() }); } return { cash_cents: balance(id, 'cash'), invested_cents: balance(id, 'invested') }; }
    ...
    if (p === '/coins/history') { const days = num(query.get('days')) || 30; const since = Date.now() - days * 86400000; const kind = (t) => (t.completion_id ? 'chores' : /^🎮/.test(t.note || '') ? 'games' : /^🎁/.test(t.note || '') ? 'prizes' : /^💵/.test(t.note || '') ? 'cashin' : 'parent'); const rows = coins.filter((t) => new Date(t.created_at.replace(' ', 'T') + 'Z').getTime() >= since).slice().sort((a, b) => b.id - a.id).map((t) => { const m = members.find((x) => x.id === t.member_id) || {}; return { ...t, member_name: m.name, emoji: m.emoji, color: m.color, kind: kind(t) }; }); const kids = []; for (const t of rows) { let k = kids.find((x) => x.member_id === t.member_id); if (!k) { k = { member_id: t.member_id, name: t.member_name, emoji: t.emoji, color: t.color, chores: 0, games: 0, prizes: 0, cashin: 0, parent: 0, net: 0 }; kids.push(k); } k[t.kind] += t.amount; k.net += t.amount; } return { days, kids, transactions: rows }; }
    ...
    if (p === '/games/session') { const memberId = num(body.member_id); const gate = choreGate(memberId); if (!gate.ok) return { status: 403, body: { error: `Finish your ${gate.period} chores first: ${gate.missing.join(', ')}` } }; const bal = coinBalance(memberId); if (bal <= 0) return { status: 402, body: { error: `Out of ${settings.coin_name}` } }; const id = nextId++; const row = { id, member_id: memberId, amount: 0, note: `🎮 ${body.game} · 0 min`, created_at: nowIso() }; coins.push(row); sessions.set(id, row); return { id, rate: rateNow(), coins: bal }; }
    if (seg[0] === 'games' && seg[1] === 'session' && seg[3] === 'tick') { const row = sessions.get(num(seg[2])); if (row) { row.amount = -Math.round((rateNow() * num(body.seconds) / 60) * 100) / 100; row.note = `🎮 game · ${Math.round(num(body.seconds) / 6) / 10} min`; } const c = row ? coinBalance(row.member_id) : 0; return { coins: c, out: c <= 0, reason: c <= 0 ? 'out_of_coins' : null }; }
