    ...
    html += `<div class="card"><h2>Today <span class="meta">${parseYmd(today.date).toLocaleDateString([], { weekday: 'short', month: 'short', day: 'numeric' })}</span></h2>
      ${groups.length ? groups.map((g) => `
        ...
        ${g.items.map((c) => {
          ...
          <div class="grow"><span class="${done ? 'strike' : ''}">${esc(c.title)}</span>${c.status === 'rejected' ? ' <span class="badge rejected">rejected</span>' : ''}${c.status === 'approved' ? ' <span class="badge approved">approved</span>' : ''}</div>
